...
8. For framework APIs, use the project's resolved version as the source of truth. Do not preserve historical boilerplate when the selected version no longer requires it. Remove redundant checked exceptions, unused imports and compatibility code introduced by the change when compilation/API evidence proves they are unnecessary. Prefer current supported configurer APIs and concise idioms, but do not rewrite an equivalent readable lambda or method reference solely for style.
