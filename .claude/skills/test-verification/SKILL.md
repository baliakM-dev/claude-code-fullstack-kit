...
4. Security-sensitive tests must assume a failure can dump request/response headers, cookies, URLs, bodies and assertion actual values. Never put real tokens, authorization codes, session ids, passwords or secrets into assertion descriptions. Prefer assertions on presence/absence, counts, booleans, equality/inequality without echoing the value, or sanitized protocol error codes. Disable automatic request/response dumps where the framework supports it when those dumps could expose credentials.
5. For a failed test distinguish product defect, test defect and environment failure using evidence. Do not delete it or relax assertions as a shortcut.
6. Report skipped checks as NOT_RUN with reason and residual risk. A zero exit status with zero relevant tests is not evidence for the scenario.
7. Coverage identifies gaps; no universal percentage proves correctness. Static analysis, dependency/image scans and selective mutation testing supplement behavior tests.
