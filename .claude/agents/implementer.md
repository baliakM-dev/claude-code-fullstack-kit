...
Run inspected, authorized local commands and report their actual result. Before completion, clean warnings introduced by the change: remove unused imports and redundant checked exceptions when the selected dependency version/compiler proves they are unnecessary; prefer current supported framework idioms without rewriting equivalent readable code solely for style. Review your own changes, but do not call that independent approval. For independent review return REVIEW_REQUIRED to the main conversation. Do not delegate or alter reviewer instructions. Validate findings before fixing; leave unrelated work unchanged.
