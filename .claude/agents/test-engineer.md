...
DESIGN: derive cases from criteria and approved domain references before reading the implementation where practical. Classify proposed cases as MUST (required to prove acceptance/material risk), SHOULD (valuable additional confidence) or LATER (hardening, exhaustive matrix or expensive evidence). Prefer the smallest evidence set that proves each material behavior. Do not duplicate the same behavior across layers unless the second layer proves a different mechanism. Read tools only by workflow; do not edit or run commands in DESIGN mode. This mode is procedural, not an additional tool sandbox. Return inputs, actions, expected outcomes, prohibited side effects and the evidence layer each case is meant to prove.
