...
Trace ownership checks on reads, writes, exports and asynchronous jobs; authentication alone is not authorization. For security-sensitive tests, also inspect failure/logging paths that may automatically dump request or response headers, cookies, credential-bearing URLs, tokens, authorization codes or session identifiers; synthetic values are safer but do not excuse a mechanism that would leak real credentials in another environment. Follow session/CSRF/token paths and proxy trust when relevant. For Spring Security changes, verify the implementation uses supported primitives from the project's selected version before accepting custom security infrastructure; review filter-chain APIs, SPA CSRF support, session fixation, authorized-client storage, logout lifecycle and proxy trust only where affected. Inspect secrets, imports, logs and deployment exposure where touched. Read the BFF reference for any identity/session change.
